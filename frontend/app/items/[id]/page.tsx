    ...
    <PublicLayout>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
        ...
        <button onClick={() => router.back()}
          className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-white mb-6 transition-colors active:scale-95">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6 lg:gap-10">
          ...
          <motion.div
            ...
            {/* Main Image */}
            <div className="relative bg-gray-100 dark:bg-gray-800 rounded-2xl overflow-hidden h-80 sm:h-96 lg:h-[420px]">
          ...
          {/* Right: Details */}
          <motion.div
            initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, ease: [0.16, 1, 0.3, 1], delay: 0.1 }}
            className="lg:col-span-2 space-y-5">
            {/* Title & meta */}
            <div>
              <div className="flex items-center gap-2 mb-2">
                <span className="text-2xl">{catIcon}</span>
                <span className="text-sm text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded-full">{item.category}</span>
                {item.subcategory && (
                  <span className="text-xs text-gray-400 dark:text-gray-500">/ {item.subcategory}</span>
                )}
              </div>
            ...
            <div className="card p-5 space-y-3">
              {[
                ...
                <div key={detail.label} className="flex items-start gap-3">
                  ...
                  <div>
                    <span className="text-xs text-gray-400 dark:text-gray-500 block">{detail.label}</span>
                    <span className="text-sm font-medium text-gray-900 dark:text-gray-100">{detail.value}</span>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="card p-5">
              <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Posted by</h3>
              <Link href={`/profile/${item.user?.id}`} className="flex items-center gap-3 group">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 dark:text-gray-500">Member since {formatDate(item.user?.createdAt, 'MMM yyyy')}</p>
                </div>
              </Link>
              {item.showContactInfo && item.user?.phone && (
                <div className="mt-3 pt-3 border-t border-gray-50 dark:border-gray-800">
                  <a href={`tel:${item.user.phone}`}
                    className="flex items-center gap-2 text-sm text-secondary-600 dark:text-emerald-400 font-medium hover:underline">
            ...
            <div className="space-y-3">
              ...
              {/* Owner controls */}
              {isOwner && (
                <div className="card p-4 space-y-2 bg-blue-50 dark:bg-primary-500/10 border-blue-100 dark:border-primary-500/20">
                  <p className="text-xs font-semibold text-blue-700 dark:text-primary-300 mb-2">Your Item — Update Status</p>
                  ...
                  <Link href={`/items/${id}/edit`}
                    className="block text-center text-xs text-primary-600 dark:text-primary-400 hover:underline mt-2">
                    Edit Item Details
                  </Link>
                </div>
              )}
              ...
              {!isOwner && isLoggedIn && (
                <button onClick={() => setReportOpen(true)}
                  className="w-full flex items-center justify-center gap-1.5 text-xs text-gray-400 dark:text-gray-500 hover:text-red-500 transition-colors py-2">
            ...
            {/* Verification notice */}
            {isFound && (
              <div className="card p-4 bg-amber-50 dark:bg-amber-500/10 border-amber-100 dark:border-amber-500/20">
                <div className="flex items-start gap-2">
                  <Shield size={16} className="text-amber-600 dark:text-amber-400 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="text-xs font-semibold text-amber-800 dark:text-amber-300">Ownership Verification</p>
            ...
            <div className="space-y-2">
              {['FAKE', 'INAPPROPRIATE', 'SPAM', 'WRONG_CATEGORY', 'OTHER'].map(reason => (
                <button key={reason} onClick={() => handleReport(reason)}
                  className="w-full text-left px-4 py-3 rounded-xl border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-300 hover:border-red-300 dark:hover:border-red-500/40 hover:bg-red-50 dark:hover:bg-red-500/10 text-sm transition-all">
                  {reason.replace('_', ' ')}
                </button>
              ))}
            </div>
            <button onClick={() => setReportOpen(false)}
              className="mt-4 w-full text-sm text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300">Cancel</button>
          </motion.div>
        </div>
      )}
    </PublicLayout>
